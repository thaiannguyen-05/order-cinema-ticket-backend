...
describe('EmailConsumer', () => {
  it('delegates queue handlers to email service', async () => {
    ...
    await consumer.sendResetPasswordEmail({ email: 'a@example.com', token: 'token-1' });

    expect(emailService.sendUserConfirmation).toHaveBeenCalledWith('a@example.com', '123456');
    expect(emailService.sendResetPasswordEmail).toHaveBeenCalledWith('a@example.com', 'token-1');
  });
});
